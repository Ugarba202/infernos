...
async fn test_mock_backend_create_and_settle() {
    ...
    let invoice = backend.create_invoice(amount, "Test invoice").await.expect("Failed to create invoice");
    ...
    let is_settled = backend.is_invoice_settled(&invoice.payment_hash).await.expect("Failed to check settlement");
    assert!(!is_settled);
    ...
    let is_settled_after = backend.is_invoice_settled(&invoice.payment_hash).await.expect("Failed to check settlement");
