...
fn node_config_deserializes_from_toml() {
    ...
    assert_eq!(
        config.default_price_sats,
        infernos::common::types::Satoshis(25)
    );
}
...
fn client_config_deserializes_from_toml() {
    ...
    assert_eq!(
        config.max_budget_sats,
        Some(infernos::common::types::Satoshis(5000))
    );
}
...
fn node_config_defaults_are_applied() {
    ...
    assert_eq!(
        config.default_price_sats,
        infernos::common::types::Satoshis(10)
    );
}
...
fn client_config_defaults_are_applied() {
    ...
    assert_eq!(
        config.max_budget_sats,
        Some(infernos::common::types::Satoshis(1000))
    );
}
